...
下部バーには Escape、バッテリー、Quit、Settings があります。
...
左 Alt + Space & Unicode キーボード \\
右 Alt/Opt + C / V & 選択範囲をコピー／貼り付け \\
\end{keytable}

Ctrl+Shift+B は Emacs などのターミナルプログラムへそのまま送られます。
...
\topic{設定と画面の調整}
下部バー右端の \key{Settings} をタップするか、\key{右 Alt/Opt+Space} を押します。
バーが非表示でも使えます。\key{左 Alt+Space} は Unicode キーボードを開きます。
...
下部バーは読み取り専用の Linux 電源インターフェースから1分ごとにバッテリー状態を読みます。
\code{inkline-battery} は残量と充電状態を表示し、\code{inkline-battery --percentage}
または \code{~/inkline battery} は Inkline の無色 Bash プロンプト用の短い値を表示します。

\topic{Unicode と入力方式}
\key{左 Alt+Space} で Unicode キーボードを開き、文字をタップして入力します。
...
\key{Open typewriter} を開きます。Send keyboard が有効ならライブ送信は自動で始まります。
一時停止中は \key{PAUSED · USB OUTPUT OFF} が黒地で明確に表示され、送信中は別の表示になります。
...
\key{Unicode} または左 Alt+Space で開く文字パレットも USB 出力に使えます。

Typewriter はローカル文書を RAM に保持するエディターです。通常の入力は文書を編集し、
Live が有効なら確定した文字を接続先にも送ります。コピー、切り取り、貼り付け、選択、
2 本指スクロールはローカル文書に作用します。\key{Files \& send} では拡張子を問わず
UTF-8 文書を読み書きし、毎秒 5--80 文字で全文を送信または停止できます。
編集中はストレージへ書きません。Save、または Inkline の正常終了時だけ書き込みます。
無題文書は終了時に隠し名 \path{~/.inkline-typewriter-draft} で復旧用に保存されます。
\key{Exit typewriter} で USB 設定へ戻ります。

\topic{ファイル送信とネットワーク復旧}
コマンド \code{inkline-usb} と \code{keyboard-send} は \path{~/.local/bin}
に入ります。\code{~/inkline usb}、\code{~/inkline type}、\code{inkline-type}
は互換エイリアスとして使えます。
