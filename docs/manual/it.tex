...
\begin{notice}
\key{Ctrl+Alt+T} apre Inkline direttamente dal tablet.\par
...
Se un'app avviata da una scorciatoia occupa lo schermo, \key{Ctrl+Alt+T} torna a Inkline.
Per il recupero d'emergenza, tieni premuto \key{Ctrl+Alt+Backspace per due secondi}.
...
\key{Ctrl+Alt+E} apre Emacs in uno slot libero.
...
\begin{notice}
\key{Ctrl+Alt+T è permanente.} Non si può registrare né rimuovere.
...
\key{Emergenza:} tieni Ctrl+Alt+Backspace per due secondi per fermare l'app
