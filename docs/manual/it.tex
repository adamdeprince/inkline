...
\key{Open typewriter}. Porta il cursore in un documento sul computer e tocca
\key{Start}. La modalità si apre sempre in pausa. \key{Escape} o \key{Stop}
...
\key{Unicode} o Alt+Space apre il selettore di caratteri per l'uscita USB.

L'anteprima recente rimane in RAM; non è un documento salvato né una copia
dell'editor ricevente. Frecce e comandi di modifica agiscono su quell'editor.
Alt/Opt destro+C copia l'anteprima, V invia gli appunti Inkline. Taglia nell'editor
ricevente. Una conferma USB non garantisce il contenuto mostrato.

\topic{Inviare file e ripristinare la rete}
I comandi sono in \path{~/.local/bin}. Nelle sessioni SSH standard puoi usare anche
\code{~/inkline usb} e \code{~/inkline type}. Gli strumenti Outpost rimangono intatti.
\UsbExamples
