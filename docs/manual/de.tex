...
zurück zu den Notizbüchern. Unten stehen Escape, Akkustand, Quit und Settings.
...
Rechte Alt/Opt + Leertaste & Settings \\
Linke Alt + Leertaste & Unicode-Tastatur \\
Rechte Alt/Opt + C / V & Auswahl kopieren / einfügen \\
\end{keytable}

Strg+Umschalt+B wird unverändert an Terminalprogramme wie Emacs weitergegeben.
...
Tippen Sie rechts unten auf \key{Settings} oder drücken Sie
\key{rechte Alt/Opt+Leertaste}; das funktioniert auch bei ausgeblendeter Leiste.
\key{Linke Alt+Leertaste} öffnet die Unicode-Tastatur.
...
Die untere Leiste liest den Akkustand einmal pro Minute aus der nur lesbaren
Linux-Stromversorgungsschnittstelle. \code{inkline-battery} zeigt Ladestand und
Ladezustand; \code{inkline-battery --percentage} oder \code{~/inkline battery}
liefert die kurze Anzeige. Der Installer nutzt sie nur in der farblosen Inkline-Bash-Eingabeaufforderung.

\topic{Unicode und Eingabemethoden}
\key{Linke Alt+Leertaste} öffnet die Unicode-Tastatur. Ein Zeichen antippen, um es
...
und öffnen Sie \key{Open typewriter}. Ist Send keyboard aktiv, beginnt die
Live-Ausgabe automatisch. Im Pausezustand zeigt der Editor deutlich
\key{PAUSED · USB OUTPUT OFF}; die Live-Ausgabe hat eine eigene umrandete Anzeige.
...
oder linke Alt+Leertaste öffnet die Zeichenauswahl für den USB-Empfänger.

Typewriter ist ein lokaler Editor; sein Dokument bleibt während Inkline im RAM.
Normales Tippen bearbeitet das Dokument und sendet bei aktivem Live-Modus jedes
bestätigte Zeichen an den Rechner. Kopieren, Ausschneiden, Einfügen, Auswahl und
Scrollen wirken im lokalen Editor. \key{Files \& send} lädt und speichert UTF-8
unter beliebigen Dateinamen, wählt 5--80 Zeichen pro Sekunde, sendet das ganze
Dokument und kann die Übertragung stoppen. Erst Save oder das normale Beenden
schreibt das Dokument. Ohne Dateinamen dient beim Beenden die verborgene Datei
\path{~/.inkline-typewriter-draft} zur Wiederherstellung. \key{Exit typewriter}
kehrt zu den USB-Einstellungen zurück.

\topic{Dateien senden und Netzwerk wiederherstellen}
Die Befehle \code{inkline-usb} und \code{keyboard-send} liegen in
\path{~/.local/bin}. \code{~/inkline usb}, \code{~/inkline type} und
\code{inkline-type} bleiben kompatible Aliase. Outpost-Werkzeuge bleiben erhalten.
\UsbExamples
