...
\key{Open typewriter}。在電腦上選取文件輸入位置，再點平板上的\key{Start}。
打字機始終以暫停狀態開啟。\key{Escape} 或\key{Stop} 暫停並釋放按鍵。
...
選擇\key{Off} 返回普通美式輸入。\key{Unicode} 或 Alt+Space 開啟的字元面板也向 USB 接收端輸入。

最近輸入的預覽僅存在 RAM 中，並非儲存的文件或接收編輯器的副本。
方向鍵和編輯快捷鍵操作接收端。右 Alt/Opt+C 複製預覽，V 傳送 Inkline 剪貼簿。
剪下應在接收編輯器中完成。USB 確認不能證明編輯器實際顯示了哪些內容。

\topic{傳送檔案與恢復網路}
安裝器將命令放入 \path{~/.local/bin}。一般 SSH 中也可使用
\code{~/inkline usb} 和 \code{~/inkline type}。Outpost 的獨立工具保持原樣。
\UsbExamples
