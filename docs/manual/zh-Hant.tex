...
\begin{notice}
按 \key{Ctrl+Opt+Alt+T}，即可直接在平板上打開 Inkline。\par
...
如果快捷鍵啟動的應用佔用了屏幕，按 \key{Ctrl+Opt+Alt+T} 返回 Inkline。
緊急恢復時，\key{按住 Ctrl+Opt+Alt+Backspace 兩秒}。
...
所有全域快捷鍵現在使用 Ctrl+Opt+Alt，包括 T 和長按 Backspace 的緊急復原組合。Folio 使用獨立的 Opt 鍵；USB 鍵盤使用 Windows/Command（Super）鍵。一般 Ctrl+Alt 仍可用於 Emacs。升級後請結束並重新開啟 Inkline；安裝會保留已開啟的終端機。
...
\key{Ctrl+Opt+Alt+E} 在空閒槽位打開 Emacs。
...
\begin{notice}
\key{Ctrl+Opt+Alt+T 是固定快捷鍵，不能註冊或取消註冊。}
它會停止原生快捷鍵應用，並返回現有終端。
請使用物理 Ctrl 鍵；Caps Lock 映射只在 Inkline 內部生效。
\end{notice}

\key{緊急恢復：}按住 Ctrl+Opt+Alt+Backspace 兩秒，停止快捷鍵應用及其子程序並重新啟動 Inkline。
