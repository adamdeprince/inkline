...
\begin{notice}
\key{Ctrl+Alt+T} opens Inkline from the tablet itself.\par
...
If a shortcut app owns the screen, \key{Ctrl+Alt+T} returns to Inkline.
For emergency recovery, hold \key{Ctrl+Alt+Backspace for two seconds}.
...
\key{Ctrl+Alt+E} opens Emacs in a free slot.
...
\begin{notice}
\key{Ctrl+Alt+T is permanent.} It cannot be registered or deregistered.
...
\key{Emergency:} hold Ctrl+Alt+Backspace for two seconds to stop the shortcut
