...
profile below, then tap \key{Open typewriter}. Focus a document on the computer
and tap \key{Start} on the tablet. The typewriter always starts paused.
...
Choose \key{Off} to return to normal US input. \key{Unicode} or Alt+Space
opens the character picker and sends selected characters to the USB host.

The recent-text preview stays in RAM; it is not a saved document or a copy of
the receiving editor. Arrow keys and editing shortcuts act on that editor.
Right Alt/Opt+C copies the preview; V sends Inkline's clipboard. Cut belongs
in the receiving editor. A USB acknowledgement cannot confirm the editor's
actual contents.

\topic{Send a file; restore networking}
The installer adds \code{inkline-usb} and \code{inkline-type} to
\path{~/.local/bin}. Stock SSH sessions can use \code{~/inkline usb} and
\code{~/inkline type} instead. The separate Outpost tools remain installed.

\begin{shell}
inkline-usb status
inkline-usb send-keyboard
inkline-type --file notes.txt --profile windows --dry-run
inkline-type --file notes.txt --profile windows --start-delay 3
inkline-type --stdin --profile linux < notes.txt
inkline-type --key ctrl+a --key backspace
inkline-type --stop
inkline-usb network
\end{shell}
