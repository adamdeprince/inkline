...
最近输入的预览仅存在 RAM 中，并非保存的文档或接收编辑器的副本。
方向键和编辑快捷键操作接收端。右 Alt/Opt+C 复制预览，V 发送 Inkline 剪贴板。
剪切应在接收编辑器中完成。USB 确认不能证明编辑器实际显示了哪些内容。

\topic{发送文件与恢复网络}
安装器将命令放入 \path{~/.local/bin}。普通 SSH 中也可使用
\code{~/inkline usb} 和 \code{~/inkline type}。Outpost 的独立工具保持原样。
\UsbExamples
