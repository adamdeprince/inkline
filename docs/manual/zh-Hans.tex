...
\begin{notice}
按 \key{Ctrl+Opt+Alt+T}，即可直接在平板上打开 Inkline。\par
...
如果快捷键启动的应用占用了屏幕，按 \key{Ctrl+Opt+Alt+T} 返回 Inkline。
紧急恢复时，\key{按住 Ctrl+Opt+Alt+Backspace 两秒}。
...
所有全局快捷键现在使用 Ctrl+Opt+Alt，包括 T 和长按 Backspace 的紧急恢复组合。Folio 使用独立的 Opt 键；USB 键盘使用 Windows/Command（Super）键。普通 Ctrl+Alt 仍可用于 Emacs。升级后请退出并重新打开 Inkline；安装会保留已打开的终端。
...
\key{Ctrl+Opt+Alt+E} 在空闲槽位打开 Emacs。
...
\begin{notice}
\key{Ctrl+Opt+Alt+T 是固定快捷键，不能注册或注销。}
它会停止原生快捷键应用，并返回现有终端。
请使用物理 Ctrl 键；Caps Lock 映射只在 Inkline 内部生效。
\end{notice}

\key{紧急恢复：}按住 Ctrl+Opt+Alt+Backspace 两秒，停止快捷键应用及其子进程并重新启动 Inkline。
