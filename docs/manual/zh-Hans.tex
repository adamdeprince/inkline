...
\begin{notice}
按 \key{Ctrl+Alt+T}，即可直接在平板上打开 Inkline。\par
...
如果快捷键启动的应用占用了屏幕，按 \key{Ctrl+Alt+T} 返回 Inkline。
紧急恢复时，\key{按住 Ctrl+Alt+Backspace 两秒}。
...
\begin{notice}
\key{Ctrl+Alt+T 是固定快捷键，不能注册或注销。}
它会停止原生快捷键应用，并返回现有终端。
请使用物理 Ctrl 键；Caps Lock 映射只在 Inkline 内部生效。
\end{notice}

\key{紧急恢复：}按住 Ctrl+Alt+Backspace 两秒，停止快捷键应用及其子进程并重新启动 Inkline。
