...
底栏包含 Escape、电池状态、Quit 和 Settings。
...
左 Alt + Space & Unicode 键盘 \\
右 Alt/Opt + C / V & 复制选区／粘贴 \\
\end{keytable}

Ctrl+Shift+B 会原样传给终端程序，包括 Emacs。
...
\topic{设置与显示}
点按底栏最右侧的 \key{Settings}，或按 \key{右 Alt/Opt+Space}；底栏隐藏时也可使用。
\key{左 Alt+Space} 打开 Unicode 键盘。实心选项表示当前状态，虚线边框表示键盘焦点。
...
底栏每分钟从只读的 Linux 电源接口读取一次电池状态。\code{inkline-battery}
显示电量和充电状态；\code{inkline-battery --percentage} 或
\code{~/inkline battery} 显示 Inkline 无颜色 Bash 提示符所用的简短数值。

\topic{Unicode 与输入法}
按 \key{左 Alt+Space} 打开 Unicode 键盘。点按字符即可输入，键盘保持打开，方便连续输入。
...
\key{Open typewriter}。Send keyboard 已启用时会自动开始实时发送。
暂停时编辑器以黑底清楚显示 \key{PAUSED · USB OUTPUT OFF}；实时发送使用另一种状态标志。
\key{Escape} 或\key{Stop} 暂停并释放按键。
...
选择\key{Off} 返回普通美式输入。\key{Unicode} 或左 Alt+Space 打开的字符面板也向 USB 接收端输入。
