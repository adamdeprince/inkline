...
La barra inferior contiene Escape, batería, Quit y Settings.
...
Alt/Opt derecha + Space & Settings \\
Alt izquierda + Space & Teclado Unicode \\
Alt/Opt derecha + C / V & Copiar selección / pegar \\
\end{keytable}

Ctrl+Shift+B se envía sin cambios a los programas, incluido Emacs.
...
Toque \key{Settings}, a la derecha de la barra, o pulse
\key{Alt/Opt derecha+Space}; funciona incluso con la barra oculta.
\key{Alt izquierda+Space} abre el teclado Unicode.
...
La barra inferior lee la batería una vez por minuto desde la interfaz de energía
de Linux, que es de solo lectura. \code{inkline-battery} muestra porcentaje y
estado de carga; \code{inkline-battery --percentage} o \code{~/inkline battery}
muestra el valor compacto usado en el indicador Bash sin color de Inkline.

\topic{Unicode y métodos de entrada}
\key{Alt izquierda+Space} abre el teclado Unicode. Toque un carácter para insertarlo;
...
\key{Open typewriter}. Con Send keyboard activo, la salida en vivo comienza
automáticamente. En pausa, el editor muestra claramente
\key{PAUSED · USB OUTPUT OFF}; la salida activa tiene un indicador distinto.
\key{Escape} o \key{Stop} detiene el envío y libera las
...
Alt izquierda+Space abre el selector de caracteres para la salida USB.
