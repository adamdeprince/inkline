...
\begin{notice}
\key{Ctrl+Opt+Alt+T} abre Inkline desde la propia tableta.\par
...
Si una aplicación lanzada por atajo ocupa la pantalla, \key{Ctrl+Opt+Alt+T} vuelve a Inkline.
En caso de emergencia, mantenga \key{Ctrl+Opt+Alt+Backspace durante dos segundos}.
...
Todos los atajos globales usan ahora Ctrl+Opt+Alt, incluidos T y la recuperación manteniendo Backspace. Use la tecla Opt independiente del Folio; en teclados USB, use Windows/Command (Super). Ctrl+Alt sigue disponible para Emacs. Cierre y vuelva a abrir Inkline tras actualizar; la instalación conserva los terminales abiertos.
...
\key{Ctrl+Opt+Alt+E} abre Emacs en una posición libre.
...
\begin{notice}
\key{Ctrl+Opt+Alt+T es permanente.} No se puede registrar ni eliminar.
Detiene la aplicación nativa y devuelve las terminales existentes.
Use una tecla Ctrl física; la reasignación de Caps Lock solo actúa dentro de Inkline.
\end{notice}

\key{Emergencia:} mantenga Ctrl+Opt+Alt+Backspace dos segundos para detener la
