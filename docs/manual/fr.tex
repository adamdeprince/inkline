...
\begin{notice}
\key{Ctrl+Opt+Alt+T} ouvre Inkline directement sur la tablette.\par
...
Si une application de raccourci occupe l'écran, \key{Ctrl+Opt+Alt+T} revient à Inkline.
En urgence, maintenez \key{Ctrl+Opt+Alt+Backspace pendant deux secondes}.
...
\key{Ctrl+Opt+Alt+E} ouvre Emacs dans un emplacement libre.
...
\begin{notice}
\key{Ctrl+Opt+Alt+T est permanent.} Impossible de l'enregistrer ou de le retirer.
Il arrête l'application native et retrouve les terminaux existants.
Utilisez une vraie touche Ctrl ; le remappage de Caps Lock reste local à Inkline.
\end{notice}

\key{Urgence :} maintenez Ctrl+Opt+Alt+Backspace deux secondes pour arrêter
