...
aux carnets. La barre du bas propose Escape, batterie, Quit et Settings.
...
Alt/Opt droite + Space & Settings \\
Alt gauche + Space & Clavier Unicode \\
Alt/Opt droite + C / V & Copier la sélection / coller \\
\end{keytable}

Ctrl+Shift+B est transmis sans modification aux programmes, dont Emacs.
...
Touchez \key{Settings}, à droite de la barre, ou faites
\key{Alt/Opt droite+Space}, même si la barre est masquée.
\key{Alt gauche+Space} ouvre le clavier Unicode.
...
La barre du bas lit la batterie une fois par minute depuis l'interface
d'alimentation Linux en lecture seule. \code{inkline-battery} affiche le
pourcentage et la charge ; \code{inkline-battery --percentage} ou
\code{~/inkline battery} donne la valeur courte de l'invite Bash sans couleur d'Inkline.

\topic{Unicode et méthodes de saisie}
\key{Alt gauche+Space} ouvre le clavier Unicode. Touchez un caractère pour l'insérer ;
...
ouvrez \key{Open typewriter}. Si Send keyboard est actif, l'envoi direct démarre
automatiquement. En pause, l'éditeur affiche clairement
\key{PAUSED · USB OUTPUT OFF} ; l'envoi actif a un indicateur distinct.
...
\key{Unicode} ou Alt gauche+Space ouvre le sélecteur de caractères pour la sortie USB.

Typewriter est un éditeur local dont le document reste en RAM pendant Inkline.
La frappe modifie ce document et, en mode direct, envoie chaque caractère validé
à l'ordinateur. Copier, couper, coller, sélectionner et défiler agissent dans
l'éditeur local. \key{Files \& send} charge ou enregistre l'UTF-8 sous tout nom,
choisit 5--80 caractères par seconde, envoie tout le document et arrête l'envoi.
Seuls Save et la fermeture normale écrivent le document. Sans nom, le fichier
caché \path{~/.inkline-typewriter-draft} sert à la récupération lors de la sortie.
\key{Exit typewriter} revient aux réglages USB.

\topic{Envoyer un fichier, rétablir le réseau}
Les commandes \code{inkline-usb} et \code{keyboard-send} sont dans
\path{~/.local/bin}. \code{~/inkline usb}, \code{~/inkline type} et
\code{inkline-type} restent des alias compatibles. Les outils Outpost restent intacts.
\UsbExamples
