...
A pré-visualização recente fica na RAM; não é um documento guardado nem uma
cópia do editor recetor. Setas e comandos de edição atuam nesse editor.
Alt/Opt direito+C copia a pré-visualização; V envia a área de transferência do
Inkline. Corte no editor recetor. Uma confirmação USB não comprova o conteúdo exibido.

\topic{Enviar ficheiros e restaurar a rede}
Os comandos ficam em \path{~/.local/bin}. No SSH habitual também pode usar
\code{~/inkline usb} e \code{~/inkline type}. As ferramentas Outpost ficam intactas.
\UsbExamples
