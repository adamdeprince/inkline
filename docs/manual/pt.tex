...
A barra inferior contém Escape, bateria, Quit e Settings.
...
Alt/Opt direita + Space & Settings \\
Alt esquerda + Space & Teclado Unicode \\
Alt/Opt direita + C / V & Copiar seleção / colar \\
\end{keytable}

Ctrl+Shift+B é enviado sem alterações aos programas, incluindo o Emacs.
...
Toque em \key{Settings}, à direita da barra, ou prima
\key{Alt/Opt direita+Space}; funciona mesmo com a barra oculta.
\key{Alt esquerda+Space} abre o teclado Unicode.
...
A barra inferior lê a bateria uma vez por minuto pela interface de energia
Linux só de leitura. \code{inkline-battery} mostra percentagem e carregamento;
\code{inkline-battery --percentage} ou \code{~/inkline battery} mostra o valor
curto usado no prompt Bash sem cores do Inkline.

\topic{Unicode e métodos de entrada}
\key{Alt esquerda+Space} abre o teclado Unicode. Toque num caráter para o inserir;
...
\key{Open typewriter}. Com Send keyboard ativo, o envio em direto começa
automaticamente. Em pausa, o editor mostra claramente
\key{PAUSED · USB OUTPUT OFF}; o envio ativo tem um indicador distinto.
\key{Escape} ou \key{Stop} interrompe o envio
...
Alt esquerda+Space abre o seletor de caracteres para a saída USB.
